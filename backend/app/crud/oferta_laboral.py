...
def get_ofertas(db: Session, categoria_id=None, empresa_id=None, estado=None, ubicacion=None, skip=0, limit=20):
    ...
    if empresa_id:
        try:
            ...
            pass
    if estado:
        query = query.filter(OfertaLaboral.estado.ilike(estado))

    if ubicacion:
        query = query.filter(OfertaLaboral.ubicacion == ubicacion)
